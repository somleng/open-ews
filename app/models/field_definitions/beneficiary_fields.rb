module FieldDefinitions
  ...
  BeneficiaryFields = Collection.new([
    Field.new(name: "status", column: beneficiaries[:status], schema: FilterTypes::ListType.define(:string, Beneficiary.status.values), description: "Must be one of #{Beneficiary.status.values.map { |t| "`#{t}`" }.join(", ")}."),
    Field.new(name: "gender", column: beneficiaries[:gender], schema: FilterTypes::ListType.define(Types::UpcaseString, Beneficiary.gender.values), description: "Must be one of `M` or `F`."),
    Field.new(name: "disability_status", column: beneficiaries[:disability_status], schema: FilterTypes::ListType.define(:string, Beneficiary.disability_status.values), description: "Must be one of #{Beneficiary.disability_status.values.map { |t| "`#{t}`" }.join(", ")}."),
    ...
    Field.new(name: "iso_country_code", column: beneficiaries[:iso_country_code], schema: FilterTypes::ListType.define(Types::UpcaseString, Beneficiary.iso_country_code.values), description: "The [ISO 3166-1](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) country code of the beneficiary."),
